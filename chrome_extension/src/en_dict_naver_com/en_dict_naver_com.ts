...
function convertMeanListToMeanings(meanList) {
  ...
  const meaningLines = meanItems.map((meanItem) =>
    convertMeanItemToMeanLine(meanItem)
  );
...
function collateMeaningBlock(row) {
  ...
  const meanings = meanLists.map((meanList) =>
    convertMeanListToMeanings(meanList)
  );
...
function processRow(row) {
  ...
  euisuContainer.appendChild(makeImageButton(targetText));
